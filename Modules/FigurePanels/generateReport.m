...
for currentCat = unique(category)'
    ...
    for j = 1:floor(length(pouchSize)/discsPerPage + 1)
        ...
        scalePxls = round(scaleBar * settings.scale.x20);
        ...
        rectangle('Position',[2,71,tmpY-2,X],'LineWidth',2)
        ...
        export_fig([settings.outRough, settings.uniqueIdentifier, '_Report_page_' num2str(page) '_' char(tmpa)],  '-pdf', '-r600')
