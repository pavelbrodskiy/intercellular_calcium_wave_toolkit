...
function metadata = figSizeBinComposites(structMaps, labels, settings, category, specificCat)
%% Parse Inputs (Lazy)
doControl = true;
...
%% Median project maps
[medianStruct, metadata] = projectSpecific(category, structMapsReduced, maskSize, specificCat, binCutoffs);
if doControl
    [medianStructControl, metadata.nControl, metadata.conditionsControl] = medianProjectSpatialMaps(structMapsReduced(category==categorical({'control'})), category(category==categorical({'control'}))');
end

medianStruct = [medianStructControl, medianStruct];

%% Write montaged figure
[figures, metadata.intensityCutoffs] = maps2RGB(medianStruct, mapOrder, fieldNames);
imwrite(figures, [settings.outRough, settings.uniqueIdentifier, char(specificCat), '_SizeBinFigureBody.png']);
